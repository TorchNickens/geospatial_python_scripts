...
path_shapes = tkFileDialog.askdirectory(parent=root, initialdir=mainDir, title='Directory to folder directory with installation shapefiles')
if len(path_shapes) > 0:
    print("Folder selected: %s" % os.path.abspath(path_shapes))
...
for shp in shapes:
    ...
    arcpy.MakeFeatureLayer_management(path_shapes + "/" + shp_name + ".shp", shp_name + "_lyr")
    count = int(arcpy.GetCount_management(shp_name + "_lyr").getOutput(0))
    if count == 0:
        print "No features exist for " + shp_name + ".shp"
        pass
    else:
        ...
        for ds in target_ds:
            ...
            for fc in target_fc:
                ...
                if fc_name == shp_name:
                    print "Appending " + str(count) + " features from " + shp_name + ".shp to the " + fc_name + " feature class in the " + ds_name + " dataset in the GeoBASE_3101_Shell.gdb"
                    arcpy.Append_management(shp_name + "_lyr", path_gdb + "/" + ds_name + "/" + fc_name, "NO_TEST","","")
                else:
                    pass
